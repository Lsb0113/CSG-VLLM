"""
Order-view (sequential) scene graph generation using a local Qwen3-VL model.

Given three sequential street-view frames of the same road segment
(~0 m / 30 m / 60 m), this script merges them into a single unified semantic
scene graph (nodes + relations) and stores one JSON file per location.

The model is loaded in bfloat16. A GPU with at least 16 GB VRAM is
recommended for Qwen3-VL-8B.

Usage:
    python Qwen_OrderView.py
"""
...
IMAGES_DIR = "Datasets/OrderViewForward_Dataset"   # one subdirectory per location
OUTPUT_DIR = "Outputs/Qwen3/OrderViewForward"
LOG_DIR = "logs"
MAX_NEW_TOKENS = 4096
...
QUERY = f"""
你是计算机视觉领域的语义场景图专家，精通二维语义场景图的标准化构建。我将给你**三张按道路顺序排列的街景图：第1张是0米处的视角，第2张是30米处视角，第3张是60米处视角**，
        请把这三张图**融合成一个完整、统一、全局的二维语义场景图**，而不是分别生成三张图的场景图，要求如下：
...
4.  提取实体数量控制在25个以内，可以少于25个实体，根据场景可提取实体数量而定。

### 三、质量要求
1.  合并同一实体：不同图片可能存在同一个实体，可能因为视角原因显示不全，但是需要推理出哪些内容属于同一实体，减少同一实体信息冗余。
2.  100%贴合画面：所有静态实体、特征、位置关系完全对应图片内容，无捏造
3.  语义精准：实体分类、关系描述符合学术规范，严格遵循上述实体类别、空间关系、拓扑关系的限定，可直接用于模型输入
4.  结构完整：无遗漏核心静态实体、无缺失关键关系、无格式错误
"""


def build_messages(query: str, images: list) -> list:
    """Build the multi-image chat message expected by Qwen3-VL.

    Each frame is embedded directly in the message content (official format);
    the text query is appended after all image slots.
    """
    content = [
...
def main() -> None:
    ...
    error_file = open(os.path.join(LOG_DIR, "error_qwen3_OrderViewForward.txt"), "w", encoding="utf-8")
    ...
    for img_name in need_process_imgs:
        # The three frames inside each location directory are expected in
        # 0 m / 30 m / 60 m order (sorted alphabetically).
        scene_list = sorted(os.listdir(os.path.join(IMAGES_DIR, img_name)))
        print(f"----------{count}----------")

        images = [
            Image.open(os.path.join(IMAGES_DIR, img_name, f)).convert("RGB")
            for f in scene_list[:3]
        ]

        messages = build_messages(QUERY, images)
        ...
        # Save one merged scene graph per location (all three frames combined).
