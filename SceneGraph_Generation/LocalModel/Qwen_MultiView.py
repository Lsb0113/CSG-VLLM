"""
Multi-view scene graph generation using a local Qwen3-VL model.

Given four omnidirectional street-view images (left / front / right / back)
of the same location, this script merges them into a single unified semantic
...
IMAGES_DIR = "Datasets/MultiViews_Dataset"      # one subdirectory per location
OUTPUT_DIR = "Outputs/Qwen3/MultiViews"
LOG_DIR = "logs"
MAX_NEW_TOKENS = 4096
...
# right(180)]. NOTE: the generation prompt below describes the four input
# slots as left/front/right/back; verify this ordering against your own data.
VIEW_FILES = ("_90_0.jpeg", "_270_0.jpeg", "_0_0.jpeg", "_180_0.jpeg")
...
4.  提取实体数量控制在30个以内，可以少于30个实体，根据场景可提取实体数量而定。
...
def build_messages(query: str, images: list) -> list:
    ...
    Each view is embedded directly in the message content (official format);
...
def main() -> None:
    ...
    error_file = open(os.path.join(LOG_DIR, "error_qwen3_multiViews.txt"), "w", encoding="utf-8")
    ...
    for img_name in need_process_imgs:
        ...
        images = [
            Image.open(os.path.join(IMAGES_DIR, img_name, vf)).convert("RGB")
