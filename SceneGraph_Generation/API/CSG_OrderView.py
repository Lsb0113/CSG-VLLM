"""
Order-view (sequential) scene graph generation via Zhipu GLM-4.6V-Flash API.

For each location with three sequential street-view frames captured along a
road (~0 m / 30 m / 60 m), this script merges the three frames into a single
unified semantic scene graph (nodes + relations) stored as one JSON file per
location.
...
IMAGES_DIR = "Datasets/OderDatasets_interval_2"   # one subdirectory per location
OUTPUT_DIR = "Outputs/GLM/OrderViews_new"
LOG_DIR = "logs"
...
def invoke_with_image(client: ZhipuAiClient, query: str, images) -> str:
    """Send up to three images plus the text query to the VLM."""
    images = list(images)
...
def main() -> None:
    ...
    error_file = open(os.path.join(LOG_DIR, "error_GLM_OrderViews.txt"), "w", encoding="utf-8")
    ...
    for img_name in need_process_imgs:
        ...
        print(f"----------{count}----------")
        images = [Image.open(os.path.join(IMAGES_DIR, img_name, f)) for f in scene_list[:3]]
