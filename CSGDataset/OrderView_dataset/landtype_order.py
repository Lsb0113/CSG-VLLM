"""
Land-use type classification from order-view (3-frame sequential) street images.

Given 3 sequential frames along a road (0m / 30m / 60m), sends them to
GLM-4.6V-Flash and predicts the dominant land-use category.

Usage:
    python landtype_order.py
"""
...
SYSTEM_PROMPT = """
你是城市街景用地类型判读专家。系统会给你同一段道路上沿同一行进方向的3帧顺序街景：第1帧约0米、第2帧约30米、第3帧约60米。请综合3帧判断该路段最主要的用地类型。

总原则：
- 只能依据图像中清晰可见的内容，不得猜测画面外信息。
- 行人、车辆等动态对象不作为判读依据。
- 三帧是同一路段的连续画面，重复出现的地物应视为同一对象。
- 用地类型必须且只能从下列7类中选择1类。
...
def build_prompt(location_id: str) -> str:
    return f"地点编号：{location_id}\n\n请综合3帧判断该路段的主导用地类型。只输出JSON。"


def setup_logger(log_dir: Path) -> logging.Logger:
    log_dir.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger("landtype_order")
    ...
    fh = logging.FileHandler(log_dir / "build_landtype_order.log", encoding="utf-8")
    fh.setFormatter(fmt)
...
def process_location(client, sample_dir, frame_items, out_path, logger, max_retries):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            raw = invoke_with_frames(client, SYSTEM_PROMPT, prompt, frame_items, temperature=0.1)
            result = validate(parse_json_response(raw), loc_id)
            result["frames"] = [{"frame": f, "position": p, "file": fp.name}
                                for f, p, fp in frame_items]
            result["image_files"] = [fp.name for _, _, fp in frame_items]
...
def main():
    ...
    for i, (sd, out) in enumerate(pending, 1):
        ...
        try:
            frames = collect_frames(sd)
            process_location(client, sd, frames, out, logger, MAX_RETRIES)
        except Exception as exc:
            failed.append({"scene": sd.name, "error": str(exc)})
            logger.error("%s failed: %s", sd.name, exc)
    write_json_atomic(LOG_DIR / "failed_landtype_order.json", {"failed": failed})
